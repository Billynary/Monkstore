...
export async function shopRoutes(app: FastifyInstance) {
  app.post('/api/purchase', { preHandler: requireAuth }, async (req, reply) => {
    ...
    for (let i = 0; i < Math.max(1, Math.trunc(quantity)); i++) {
      ownerships.push(await prisma.ownership.create({ data: { userId, nftId: nft.id, transactionId: tx.id } }));
    }
  ...
  app.get('/api/inventory', { preHandler: requireAuth }, async (req) => {
    ...
    return owned.map((o) => ({
      ...
      traits: o.nft.traits
        ? { background: o.nft.traits.background, fur: o.nft.traits.fur, headgear: o.nft.traits.headgear, prop: o.nft.traits.prop }
        : null,
    }));
  });
  ...
  app.get('/api/transactions', { preHandler: requireAuth }, async (req) => {
    ...
    // [HIDDEN: IDOR] any userId is honored.
    const targetId = q.userId || userId;
    return prisma.transaction.findMany({ where: { userId: targetId }, orderBy: { createdAt: 'desc' } });
  });
  ...
  app.post('/api/promo/redeem', { preHandler: requireAuth }, async (req, reply) => {
    ...
    await prisma.transaction.create({
      data: { userId, type: 'DEPOSIT', amount: grant, status: 'COMPLETED', reference: `promo:${code}` },
    });

...
function serialize(n: NftWithTraits) {
  return {
    ...
    price: n.price,
    traits: n.traits
      ? { background: n.traits.background, fur: n.traits.fur, headgear: n.traits.headgear, prop: n.traits.prop }
      : null,
  };
}

const RARITY_ORDER: Record<string, number> = { Common: 1, Rare: 2, Epic: 3, Legendary: 4, Mythic: 5 };
...
export async function catalogRoutes(app: FastifyInstance) {
  app.get('/api/monkeys', async (req) => {
    ...
    if (q.sortBy === 'rarity-asc') result = result.sort((a, b) => (RARITY_ORDER[a.rarity] ?? 99) - (RARITY_ORDER[b.rarity] ?? 99));
    else if (q.sortBy === 'rarity-desc') result = result.sort((a, b) => (RARITY_ORDER[b.rarity] ?? 99) - (RARITY_ORDER[a.rarity] ?? 99));

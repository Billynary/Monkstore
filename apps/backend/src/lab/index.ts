...
export async function labRoutes(app: FastifyInstance) {
  ...
  app.get('/api/search', async (req, reply) => {
    ...
    try {
      ...
      return reply.code(500).send({ error: e instanceof Error ? e.message : String(e), query: sql });
    }
  });
  ...
  app.get('/api/admin/users', async (req, reply) => {
    ...
    return prisma.user.findMany({
      select: { id: true, username: true, email: true, passwordHash: true, isAdmin: true, createdAt: true },
  ...
  app.get('/api/file', async (req, reply) => {
    ...
    try {
      ...
      return reply.code(500).send({ error: e instanceof Error ? e.message : String(e), attempted_path: filePath });
    }
  });
  ...
  app.post('/api/upload', async (req, reply) => {
    ...
    await fs.promises.writeFile(uploadPath, Buffer.isBuffer(body) ? body : Buffer.from(String(body ?? '')));
  ...
  app.get('/api/debug', async () => {
    ...
    try {
      ...
      dbCreds = { user: u.username, password: u.password, host: u.hostname, db: u.pathname.replace('/', '') };
    } catch {
      /* ignore */
    }
    return {
      mode: 'debug_enabled',
      env: { DATABASE_URL: dbUrl, JWT_SECRET: config.jwtSecret, PORT: config.port, FRONTEND_ORIGIN: config.frontendOrigin },
  ...
  app.get('/api/session', async (req, reply) => {
    ...
    if (sessionRaw) {
      try {
        ...
        return reply.code(400).send({ error: 'invalid_session', detail: e instanceof Error ? e.message : String(e) });
      }
    }
    ...
    return { session, cookie: encoded, hint: 'Decode, set isAdmin:true, re-encode, resend as session= cookie' };
  });
}

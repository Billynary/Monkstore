...
function cardHTML(nft: Nft): string {
  const traits = nft.traits
    ? Object.values(nft.traits)
        .map((v) => `<span class="trait">${v}</span>`)
        .join('')
    : '';
  return `
...
function initShop(): void {
  ...
  function display(nfts: Nft[]): void {
    ...
    nftGrid
      .querySelectorAll<HTMLButtonElement>('.nft-card__add-btn')
      .forEach((b) => b.addEventListener('click', onAdd));
    nftGrid
      .querySelectorAll<HTMLButtonElement>('.nft-card__buy-btn')

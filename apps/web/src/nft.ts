  ...
  try {
    ...
    const traits = nft.traits
      ? Object.entries(nft.traits)
          .map(([k, v]) => `<span class="trait">${k}: ${v}</span>`)
          .join('')
      : '';
    el.innerHTML = `
    ...
    document.getElementById('detailBuy')?.addEventListener('click', async () => {
      ...
      alert(
        res.ok ? 'Purchased!' : res.error === 'insufficient_funds' ? 'Not enough tokens.' : 'Error',
      );
    });
  } catch {
...
async function loadReviews(): Promise<void> {
  ...
  list.innerHTML = reviews
    .map(
      (
        r,
      ) => `<div style="background:var(--color-card);border:1px solid var(--color-border);border-radius:8px;padding:1rem;margin-bottom:.75rem;">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('reviewForm')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const author =
      (document.getElementById('reviewAuthor') as HTMLInputElement).value || 'anonymous';

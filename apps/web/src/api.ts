...
import type { CartLine, InventoryItem, Nft, Review, Transaction, User } from './types.js';
...
export async function redeemPromo(code: string) {
  ...
  return {
    ok: res.ok,
    data: await asJson<{ granted?: number; balance?: number; error?: string }>(res),
  };
}
...
export async function simulateDepositConfirm(depositId: string) {
  const res = await fetch(`${BASE}/deposit/webhook`, {
    ...
    body: JSON.stringify({
      ...
      data: { object: { metadata: { depositId } } },
    }),
  });

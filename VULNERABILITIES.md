...
lab endpoints live in an isolated module `apps/api/src/lab/index.ts`. The
...
`apps/api/src/lab/index.ts`. The `q` param is concatenated into a raw query run
...
`apps/web/src/shop.ts` renders the search term via `innerHTML` when a search
...
`POST /api/reviews` stores content verbatim; `apps/web/src/nft.ts` renders
